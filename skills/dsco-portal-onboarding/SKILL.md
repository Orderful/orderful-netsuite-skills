...
**Watchout (AAFES):** The generic instructions talk about 846 EDI and AS2 — **ignore these for the portal bootstrap**. The AAFES note says "USE THIS TEMPLATE NOT WHAT IS LISTED IN THE INSTRUCTIONS SECTION." (Production inventory IS a scheduled 846 feed later — the CSV is only for portal steps 6–7. See `reference/aafes-dsco.md`.)
...
5. Do NOT create a new cert — reuse the shared connection (one Orderful shared station serves all DSCO customers)
...
6. **The portal inventory upload (steps 6–7) is a CSV bootstrap, not the production feed** — for AAFES it's a simple 2-column spreadsheet. Production inventory is a real, scheduled 846 EDI feed; see `reference/aafes-dsco.md` → "AAFES DSCO 846 Inventory Feed"
