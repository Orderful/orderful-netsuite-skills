...
2. Be aware: SuiteTax + REST PATCH on transactions is broken in many accounts (see `skills/netsuite-setup/SKILL.md` "Known issue: SuiteTax + REST PATCH on transactions"). A `record.submitFields` RESTlet path would be cleanest, but the SuiteApp's agent-write RESTlet does **not** currently expose a `submitFields` action (only `triggerInboundPolling` and `reprocessTransaction`) — so programmatic SO writes aren't available today. Use the NS UI / CSV fallbacks, and don't promise the user a programmatic apply unless the connector adds that action.
