...
- **The `field_value_type === 'DATE'` mismatch at `createSalesOrder.ts:871` / `:979` is a *hypothesis*, not confirmed.** It's a plausible read of a decompiled path, but the shipped connector is a locked/minified bundle, so the exact line can't be verified from a customer account — don't repeat it as established fact. What *is* verified: `_textFields` is the reliable way to get a string date onto a custom date field on the **first** save.
- **There is a second, separate failure on some builds: the post-create reload-and-resave pass.** After the SO is created correctly, the connector reloads it and re-saves it in a *verify/correct* pass — a generic, type-blind `intended != read-back → re-apply` loop (it flags `orderstatus` and native `trandate` too, not just custom dates). Because a `_textFields` date is held as an `M/D/YYYY` **string** and read back as a **Date object**, it always "mismatches", and the re-apply throws the same `INVALID_FLD_VALUE: Invalid date value (must be M/D/YYYY)` on the **second** save — *even though the first save created the SO correctly*. Tells: the SO exists and is correct, but every inbound OT is Error with that message (stored in `customrecord_orderful_transaction_error`, not on the OT), and it fires **regardless** of whether a `customrecord_orderful_edi_field_map_line` row exists. On these builds, `_textFields` is necessary but **not sufficient**.
- **Mitigation for the reload-and-resave failure: set the feature flag `isSalesOrderReloadAndResaveDisabled = true`** (see [`set-feature-flag`](../set-feature-flag/SKILL.md)). Disabling the reload-and-resave loses nothing here — that pass only ever re-flags already-correct fields, so the SO is already right after the first save.
