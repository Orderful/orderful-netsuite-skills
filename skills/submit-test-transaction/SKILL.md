...
**Related:** if the goal is to inject an inbound test that must land in a customer's **sandbox NetSuite** (with routing guardrails so it never touches prod), use [`inject-test-transaction`](../inject-test-transaction/SKILL.md) instead — this skill covers the API mechanics and payload construction; that one covers the safe end-to-end injection flow.
...
Example: Vantage Supply (unclaimed) responding to Acme Medical (claimed):
```json
{
...
```
Submit with Acme Medical's API key. Orderful delivers to Acme Medical's poller via the existing Vantage Supply→Acme Medical relationship. Validated May 2026 — see `reference/procure-to-pay.md`.
...
2. **4-layer testing strategy** (validated on Acme Medical, May 2026):
...
4. **997s may be mandatory.** Some trading partners (e.g., Vantage Supply) require 997 functional acknowledgments in both directions. Verify during relationship setup.

See `reference/procure-to-pay.md` for the full Acme Medical testing strategy.

5. **Prod-only cleanup protocol.** When the customer has no NS sandbox (e.g., Acme Medical), all test transactions land in production. Before testing:
   ...
   - Validated May 2026: Jordan (Acme Medical) confirmed he'd track test transaction numbers for later cleanup
...
- See `audit-outbound-rules` skill — **audit /v2/rules BEFORE outbound testing** to catch silent segment stripping
