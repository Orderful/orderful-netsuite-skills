...
Before writing or iterating any JSONata, run [`audit-outbound-rules`](../audit-outbound-rules/SKILL.md) for the customer × document type. Per-relationship rules at `/v2/rules` can silently **strip, INVERT, or ADD** EDI segments AFTER the SuiteApp sends — surfacing not only as "missing field" validation errors that NS-stored messages and `/v3/validate` both contradict, but also as inverted values or spurious empty segments the connector never emitted. See the ["Rule shapes beyond whitelist+REMOVE"](../audit-outbound-rules/SKILL.md#rule-shapes-beyond-whitelistremove) taxonomy (value-swap and unconditional const-stamp) and the ["Attribute the defect: connector vs. rules"](../audit-outbound-rules/SKILL.md#attribute-the-defect-connector-vs-rules) recipe. If a rule's whitelist is narrower than the partner's spec, fixing the rule is the cleaner answer than working around it in JSONata. Skipping this step is the most common source of "I'm sure I sent the right segment, why does the validator say it's missing" mysteries.
...
| Same segment, different schema at different HL levels | Some partners encode different rules for the *same* segment depending on which HL it's in. the partner's 856 TD5 is the canonical example: at the Shipment HL it expects TD502/TD503/TD505 (SCAC qualifier + code + carrier routing); at the Order HL it expects only TD506 (`shipmentOrderStatusCode: "CL"` or `"PR"` — order completion). Putting Shipment-shaped TD5 at the Order HL fires "Segment is mandatory" because the actual required field (TD506) is missing. Always check whether per-HL rule variants exist before assuming the segment shape is uniform. |
...
15. **The partner GUIDELINE — not a rule's whitelist — is the source of truth for which qualifier a slot requires.** When a validation error names a missing/invalid qualifier ("Segment is mandatory", "`X` is not a valid input"), pull the guideline's allowed values for that exact `dataPath` *first* (`GET /v2/guideline-sets/{id}/guidelines?convertToOrderfulPath=true`; accepts the `orderful-api-key`) and emit what the guideline allows. Do NOT infer the required qualifier from an outbound rule's allow-array (`/v2/rules`) — those arrays are not the spec, and editing them is a no-op if the guideline strips the qualifier downstream anyway. Real example: a shipment-level `REF` rule listed `2I`, so `REF*2I` was emitted and the rule edited repeatedly — all no-ops, because the guideline only allowed `BM`. One look at the guideline would have shown `BM` up front.
...
- **The outbound message is content-identical across multiple saved ECT JSONata versions.** If you re-save the ECT with meaningfully different JSONata and the *received* payload doesn't budge, the ECT is not the lever — Orderful rules are post-processing the output regardless of the mapping (see [`audit-outbound-rules`](../audit-outbound-rules/SKILL.md)). Stop iterating on JSONata and run the [connector-vs-rules attribution diff](../audit-outbound-rules/SKILL.md#attribute-the-defect-connector-vs-rules): reconstruct the connector's true output offline and compare against the received payload. This complements the "JSONata evaluated but nothing changed" case above — there the transform no-oped locally; here the transform works but a rule overwrites it downstream.
...
AAFES via DSCO (guideline 146865) is strict on 810 invoices — extra fields cause rejection. a teammate's JSONata v2 handles these deviations:
