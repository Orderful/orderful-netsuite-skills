...
Vantage Supply is already LIVE with another Orderful customer (partnership active since Nov 2024, HTTP webhook comm channels on both test and prod).

This proves Vantage Supply's connectivity and EDI profile work. Any new customer integrating with Vantage Supply can reference that relationship as a known-good baseline.
...
Estimated effort: 2-4 hours per transaction type for the custom script (per Orderful's NS connector engineer). The SI partner's developer is writing the scripts.
...
**⚠️ REVISED May 14, 2026 (live call with Jordan).** Original design (May 13) was more complex. Team aligned on bare-minimum first pass:
...
- **No automation until real data:** the team recommended doing the bare minimum now and avoiding automation until they have some months of real transaction data to base decisions on.
...
- **Drop-ship PO is the key test artifact.** Jordan creating one in NS production. Will send PO# + linked SO#. This is the critical input for testing the full P2P cycle.
...
**Key blocker:** Jordan creating a drop-ship PO in NS production. Once received (PO# + SO#), the full round trip can be tested.

### 855 Design Detail — v1 (simplified May 14)

**v1 scope (agreed with Jordan May 14):**
...
- SuiteApp: Installed, connector provisioned (by Orderful + the SI partner)
...
10. The requirements baseline document pattern works. A structured baseline doc (why/what's different/the #1 decision/status snapshot/doc-by-doc requirements/testing strategy/what we need/timeline/appendix) got Jordan to respond with detailed, implementable answers in <12 hours. When the earlier ad-hoc questions were thin, Jordan's answers were thin. The framework made the difference.
...
16. Custom SuiteScript delivery matches estimates. The SI developer delivered Core 4 scripts (850/855/856/810) in ~2 days after receiving Jordan's requirements. The 2-4 hours per TX type estimate was accurate. Custom fields are the long-pole — they require customer action in NS before scripts can run.
...
25. Minimal-first beats comprehensive-first for go-live speed. The May 14 meeting reduced the 855 from a 5-field design with automated hold triggers to a single free-text status field. The 856 went from Orderful EDI Carton records with carton hierarchy to just a tracking number. Total implementation surface dropped dramatically, and the customer was happier. The "2 weeks to first pass go-live" estimate is directly tied to this simplification. Over-engineering the first pass delays go-live and creates maintenance burden before anyone knows what matters.
