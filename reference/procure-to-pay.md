...
Acme Medical (Medical Application Repair & Sales) is a medical supply wholesaler buying from Vantage Supply, Inc. This is a P2P flow and the SuiteApp's first production P2P customer. Acme Medical sends outbound 850s (Purchase Orders) and receives inbound 855/856/810 back. The standard SuiteApp handles Order-to-Cash (inbound 850, outbound 855/856/810) natively; P2P requires custom SuiteScript for every transaction type.

## Key Facts

| | Acme Medical | Vantage Supply |
|---|---|---|
| Org ID | 124676 | 41414 |
| Org Type | customer | tradingPartner (unclaimed) |
| EDI Account ID | 128085 | 40366 |
| ISA (live) | ORDFLACMEMED | VANTAGEPROD |
| ISA (test) | ORDFLACMEMEDT | VANTAGETEST |
| Billing | INTEGRATED / ACTIVE (PAYS) | N/A (unclaimed) |
| ERP | NetSuite (account 1234567, prod only) | Unknown |
| Comm Channel | Poller (test: 68810, prod: 68811) | None (unclaimed) |

## Vantage Supply EDI Profile

Vantage Supply is an unclaimed trading partner with no API key and no comm channels at the org level. Connectivity is managed by Vantage Supply's own systems (AS2 to Orderful).
...
| Field | Value |
|---|---|
| Environment | Vantage Supply / Test |
...
Key insight: Vantage Supply distinguishes between **local** contracts (direct between Vantage Supply and Acme Medical) and **GPO** contracts (Group Purchasing Organization — where pricing comes through a GPO agreement). This affects the 845 NS contract structure we need to propose to Jordan — it needs a contract_type field (local vs GPO).

Vantage Supply wants 846 tested before 845. During 845 testing they will validate all 4 scenarios above.

## Existing Vantage Supply Relationships

Vantage Supply is already LIVE with Dealmed.com (org 28549, EDI account 26743). Partnership id 33652, active since Nov 2024. Dealmed uses HTTP webhook comm channels (both test and prod).

This proves Vantage Supply's connectivity and EDI profile work. Any new customer integrating with Vantage Supply can reference the Dealmed relationship as a known-good baseline.

## Transaction Set

Acme Medical to Vantage Supply relationships (all TEST status, autoSend ENABLED as of May 2026):

| Relationship ID | Direction | TX Type | System Name |
|---|---|---|---|
| 225935 | Acme Medical to Vantage Supply | 850 | 850_PURCHASE_ORDER |
| 225932 | Vantage Supply to Acme Medical | 855 | 855_PURCHASE_ORDER_ACKNOWLEDGMENT |
| 225934 | Vantage Supply to Acme Medical | 856 | 856_SHIP_NOTICE_MANIFEST |
| 225930 | Vantage Supply to Acme Medical | 810 | 810_INVOICE |
| 225941 | Acme Medical to Vantage Supply | 867 | 867_PRODUCT_TRANSFER_AND_RESALE_REPORT |
| 225938 | Vantage Supply to Acme Medical | 845 | 845_PRICE_AUTHORIZATION_ACKNOWLEDGMENT_STATUS |
| 225940 | Vantage Supply to Acme Medical | 846 | 846_INVENTORY_INQUIRY_ADVICE |

Core P2P flow: 850 out, 855/856/810 in. The 867 (Product Transfer/Resale Report) is a standard medical distribution requirement. The 845 (Price Authorization) and 846 (Inventory) are confirmed in-scope by Jordan (May 13).
...
Estimated effort: 2-4 hours per transaction type for the custom script (per a teammate Riesman-Tremonte). Abdallah (a contractor vendor Consulting) is writing the scripts.

## Drop-Ship vs Stock: The Load-Bearing Design Decision

Discovered May 12, confirmed by Jordan May 13. Analysis of Acme Medical's Vantage Supply PO data from production NS:
...
This means 87% of Acme Medical's Vantage Supply orders have goods going directly to Acme Medical's customer — never touching Acme Medical's warehouse at HQ.

### Jordan's Confirmed Requirements (May 13, 2026 — updated May 14)
...
**Stock (13%):** Do NOT auto-create Item Receipt from 856 alone. Stage shipment data, notify Acme Medical goods are inbound. Item Receipt created only after physical dock receipt confirmed by a human. No phantom inventory. Jordan confirmed (May 14) the stock path is secondary — the warehouse handling of 856s and the lack of a natural NS home for pallet/carton data was his second biggest concern.

**⚠️ Carton/packaging data — RESOLVED May 14:**
- **Drop-ship:** Jordan only cares about **tracking number** on the Item Fulfillment under Packages. Does NOT care about carton-level breakdown ("what's in each box"). Skip Orderful EDI Carton records — just write the tracking number to the IF Packages tab.
...
- **Fields on IF Packages tab:** package weight, contents description, tracking number, carton number are all visible, but Acme Medical only uses **tracking number**.
...
- Acme Medical does use NS native lot handling features, but NS doesn't enforce them in the drop-ship scenario.

**"VENDOR - Vantage Supply (VIEW ONLY)" location:** Deactivated. Was a virtual location for Vantage Supply inventory but would have required GL/accounting changes Acme Medical didn't want. Do NOT use for receiving, drop-ship, inventory tracking, or 846 updates.
...
## Confirmed Requirements by TX Type (Jordan, May 13)

### 850 — Outbound Purchase Order
- **REF*8M vs REF*IA/IT:** Acme Medical believes 723403 is their Vantage Supply account number, fits IA/IT (6 chars). Needs Vantage Supply confirmation whether 8M is the same or a separate company-level ID.
- **BEG02 routing:** DS when PO linked to SO (drop-ship), SA for stock. Drop-ship link to SO is the indicator.
- **N9*ZZ:** Unknown — needs Vantage Supply confirmation of expected value.
...
- **Item matching:** Primary = vendor code from Vendor Pricing tab on item record. Acme Medical item number NOT useful. MPN/manufacturer part number as fallback needs Vantage Supply confirmation.
- **Fees/shipping/tax:** Vantage Supply bills include processing fees, shipping fees, and sometimes tax. Vantage Supply processing fees → Acme Medical processing fees. Vantage Supply shipping → Acme Medical shipping. Tax (if any) → shipping.
...
- **⚠️ May 14 update:** Team agreed to review **historical production vendor bills** in NS for guidance on output format. Jordan confirmed historical bills exist. Match the Vendor Bill structure to what Acme Medical already has in production — don't invent a new format.

### 855 — Inbound PO Acknowledgment

**⚠️ REVISED May 14, 2026 (live call with Jordan + a teammate).** Original design (May 13) was more complex. Team aligned on bare-minimum first pass:

- **Header status:** Single **custom free-text field** on PO header for 855 acknowledgement status. Jordan agreed to create this field. No complex BAK02 status mapping for v1 — just join the 855 to the correct PO transaction, flag that an 855 was received, and show the status at header level.
- **No automation until real data:** a teammate recommended doing the bare minimum now and avoiding automation until they have some months of real transaction data to base decisions on.
- **Rejection visibility:** Jordan's main use case is knowing if a PO gets rejected. The free-text field covers this.
- **Qty/price mismatches:** Keep lightweight. Jordan said he hasn't seen Vantage Supply send different qty/price than ordered. Expects backorders are more likely than quantity shortfalls. Let the customer team handle mismatches manually if they arise.
- **Re-acks / multiple 855s:** Latest 855 wins. Jordan confirmed once he understood the 855 carries projected ship dates that can update. They want the latest value plus some indication of when it was updated.
...
- **Automation:** Jordan wants full automation, not manual maintenance.
- **Contract awareness:** Jordan didn't know Vantage Supply's 845 is contract-based. Asks US to propose the NS structure (custom fields, contract record, pricing sublist, scheduled job).
- **Target fields:** Item Purchase Price + Vendor Pricing tab → Vantage Supply price.
...
- **Warehouse aggregation:** Sum across all Vantage Supply warehouses into existing text field. Structured breakdown later if needed.
- **Update cadence:** Overwrite on each 846 receipt + capture "last updated" timestamp.
- **Item matching:** Vendor code from Vendor Pricing tab (same as 810). MPN fallback needs Vantage Supply confirmation.
- **Missing items:** Log silently. Alert only if unmatched count exceeds threshold.

### 867 — Outbound Product Transfer/Resale Report
- **DI qualifier:** Use Acme Medical NS invoice number (the invoice Acme Medical issued to its customer).
- **Trigger:** Monthly, 1st of month, previous month. Generate + hold for human review first 1-2 months, then auto-send.
- **Zero-activity:** Stub only if Vantage Supply requires it, otherwise skip + log.
- **Existing report:** Acme Medical currently uploads a report to Vantage Supply — will provide example as reference for 867 mapping.
- **DEA number:** Not used today but expanding soon. Jordan will create custom field on customer record. Needs format guidance from us.
- **Member ID (1W):** Needs Vantage Supply confirmation.

### Testing Strategy
- 4-layer approach accepted by Jordan: validate transforms → TEST stream → PendingCustomProcess staging → contained test POs.
- First real 856 against a real PO held as PendingCustomProcess until Acme Medical confirms mapping.
- SuiteScript = Full permission will be granted (needs exact role name from us).
- **⚠️ May 14 update:** Testing happens in **production** (no sandbox). Biggest complication per Jordan. Test transactions must be tracked and later deleted/cleared out. Jordan will keep a record of test transaction numbers.
- **Drop-ship PO is the key test artifact.** Jordan creating one in NS production. Will send PO# + linked SO#. This is the critical input for Abdallah to test the full P2P cycle.
- **Round-trip test plan:** Team has already done simulated round trips with Orderful responding as Vantage Supply would. Full round-trip testing depends on Vantage Supply being connected (AS2 setup pending).

## Script Status (updated May 14)

Core 4 custom SuiteScript built and deployed. Validated against Vantage Supply sample files and internally generated test samples. Scripts need updating to reflect May 14 simplifications before testing with Jordan's drop-ship PO.
...
**Key blocker:** Jordan creating a drop-ship PO in NS production. Once received (PO# + SO#), Abdallah can test the full round trip.

### 855 Design Detail — v1 (simplified May 14)

**v1 scope (agreed by Jordan + a teammate May 14):**
1. Join the 855 to the correct PO transaction
2. Flag that an 855 was received
3. Show the 855 status at header level (custom free-text field — Jordan creating)
...
|---|---|---|---|
| TBD (Jordan creating) | Free text | PO body | 855 acknowledgement status at header level |
...
## Items Awaiting Vantage Supply Confirmation

As of May 13, Jordan sent an email to Vantage Supply (Travita Dumas) with these open items:

1. Test ISA setup confirmation
2. Vantage Supply EDI testing contact (answered: Travita Dumas, Vantage Supply EDI team)
...
## Items We Owe Back to Jordan

1. ~~Can native NS PO status reflect EDI ack states?~~ **RESOLVED May 14** — agreed on custom free-text field instead. Jordan creating it.
...
## Items Jordan Owes Us (as of May 14)
...
Vantage Supply EDI contact: **Travita Dumas** (travita.dumas — confirmed via email to a teammate, Jordan, a teammate, a teammate, Abdallah, Selma)
...
- **845 test scenarios:** Travita previously provided expected 845 test scenarios (need to locate — check Abdallah/Selma/Jordan).
...
- `SAC*C*D240***600` — Surcharge: D240 (Freight), $6.00. Maps to Jordan's "shipping fees → Acme Medical shipping" rule.

### sample867.txt — Product Transfer/Resale Report
- `BPT*00*1234567*20260426*SS` — report type SS (Seller Sales)
- `DTM*090/091` — period Mar 29 – Apr 25
- `PTD*SS***CT*10051480081` — **CT = Contract Number**. Ties 867 to 845 contract system.
- `REF*DH*BG5789574` — **DEA Number is present.** Vantage Supply already uses DEA in 867.
- `REF*HI*1KM4CYL00` — Health Industry Number
- `LIN*1*MG*SMI 66022003*F1*1234567` — MG (Manufacturer's Part Number) + F1 qualifier
...
- Product code format confirmed: `DER 16481`, `DUK 1030TD`, `MMM 00135LF`, `GRA 37235`, `TEC 1100`, `NUT 1234567`, `EXE 26703`
...
Stored at `~/orderful-onboarding/acme-medical/vantage-supply-samples/Vantage Supply 846 Guide.pdf`

Guide-specified structure:
- BIA: Transaction Set Purpose Code 00 (Original), Report Type PI (Product Availability Inquiry)
- REF*IA mandatory in header — **Vantage Supply Internal Vendor Number for Acme Medical**. Same as the 850's REF*IA/IT field. Value should be Acme Medical's Vantage Supply account number.
...
1. **LIN identifier:** Guide says LIN02=UP (UPC). Sample uses VP (Vendor Part Number). Vantage Supply's actual 846 identifies items by VP, not UP. Parser should handle VP as primary.
2. **SDQ encoding:** Guide says SDQ02 = qualifier 54 (Warehouse). Sample shows `SDQ*CS*A*02` and `SDQ*CS*D*02` where the second element appears to be an availability status (A=Available, D=Discontinued), not a warehouse qualifier. Either Vantage Supply repurposed SDQ02 or the encoding differs from spec.
...
- Account: 1234567 (production only, no sandbox)
- SuiteApp: Installed, connector provisioned (by a teammate Foster / Selma from a contractor vendor)
- Sandbox: Acme Medical does not appear to have a NetSuite sandbox. This is the first known prod-only NS customer at Orderful. All testing must use Orderful's TEST stream plus NS test flags to isolate test data.
...
### Outbound 850 from Acme Medical (NetSuite-generated)
- TX 900000000: PO 1006083_Test, 2 lines (ADC-6024N blood pressure monitor $46.35, Aspen Eye Bubble $64.46)
...
### Simulated Vantage Supply Responses (generated via API, May 11 2026)
Submitted using Acme Medical API key with Vantage Supply test ISA in envelope:
...
- No guideline sets configured for Vantage Supply. Validation runs against base X12 schema only
...
| 2026-05-06 | Customer reached out. No follow-up from a contractor vendor PM after kickoff |
| 2026-05-08 | a contractor vendor tried leader flow setup (wrong direction), a teammate did training, customer confused |
| 2026-05-08 | a teammate flagged SuiteApp does not support outbound 850s natively |
...
| 2026-05-11 | a teammate takes over, audits org, generates test 855/856/810, identifies NS validation bug |
| 2026-05-12 | Abdallah delivering custom P2P scripts (target) |
| 2026-05-12 | a teammate sends requirements baseline doc to Jordan; Abdallah's batched Q&A email to Jordan with inline responses |
| 2026-05-12 | a teammate publishes "Acme Medical × Vantage Supply — EDI Requirements Baseline" on Confluence (IM space) |
| 2026-05-12 | All 7 guideline sets created and assigned in Orderful |
| 2026-05-13 05:25 PT | Jordan responds with full requirements PDF (MARS_Notes_5_13_26_Orderful.pdf) — all 7 TX types answered, §3 blocker resolved |
| 2026-05-13 06:31 PT | Travita Dumas (Vantage Supply EDI) responds with testing process, sample files (855/856/810/867/846), and AS2 cert |
| 2026-05-13 | Vantage Supply provides AS2 certificate (a prior VAN Shared AS2, expires 2027-05-07) and requests Acme Medical's AS2 details for connectivity test |
| 2026-05-13 | Vantage Supply clarifies 845 testing: 4 scenarios (new/full local, new/full GPO, change/add/delete local, change/add/delete GPO) each with header/items/eligible end users |
| 2026-05-13 | a teammate sends Vantage Supply (Travita) consolidated email with 11 open questions — 2 block first test 850 (REF*8M, N9*ZZ), rest are edge cases |
| 2026-05-13 | a teammate sends Jordan email with PDF status report (pending Abdallah's custom field list) |
...
| 2026-05-13 | Abdallah drafts email to Jordan with 855 field design (BAK02 status mapping, 5 custom fields), 856 carton packaging question, 810 charge mapping question |
| 2026-05-13 | Status update posted to #acme-medical_mts Slack channel |
| 2026-05-14 | a teammate sends Jordan technical spec email: 855/856/810 fields, logic, and open decisions |
| 2026-05-14 | Live call with Jordan + a teammate + Abdallah + a teammate. Core 4 (850/855/856/810) built and validated against samples. Aligned on minimal v1 approach |
| 2026-05-14 | **855 simplified:** bare-minimum free-text field, no automation until real data. a teammate's recommendation accepted by Jordan |
...
| 2026-05-14 | Jordan to create drop-ship PO in NS production and send PO# + SO# for testing |
...
| 2026-05-14 | **Near-term plan agreed:** P2P workflow first → drop-ship PO from Jordan → test with samples → round trip → then 845/846/867. First pass go-live: ~2 weeks (a teammate estimate) |
...
2. Kickoff SOP gap. The Customer Kickoff SOP does not have a decision gate for P2P vs O2C. When the flow is P2P, the kickoff should immediately flag: (a) custom script work required, (b) estimated hours per TX type, (c) who is building it (a contractor vendor, Orderful, customer). The Northwind Apparel-style pre-call prep should include a flow direction check.

3. Leader/follower confusion. Acme Medical is a follower (receives Vantage Supply's guidelines) but sends outbound 850s. Multiple team members confused follower with only receives documents. Leader/follower refers to who controls the EDI spec, not the direction of the PO.
...
10. The requirements baseline document pattern works. a teammate's doc structure (why/what's different/the #1 decision/status snapshot/doc-by-doc requirements/testing strategy/what we need/timeline/appendix) got Jordan to respond with detailed, implementable answers in <12 hours. When Abdallah's earlier questions were thin, Jordan's answers were thin. The framework made the difference.

11. Jordan is a capable NS user. His "I assume" answers in the May 12 email were thin because the questions were thin. Given a proper framework, he delivered detailed answers including drop-ship completion mechanics, Vendor Pricing tab references, GL accounting implications, and AP workflow preferences.

12. 845 is a contract subsystem, not a price update. Vantage Supply's 845 is contract-based with status codes, effective dates, quantity-break pricing, proposed vs active contracts. Vantage Supply's 845 test scenarios include local contracts, GPO contracts, and change/add/delete operations with header/items/eligible end users. Building this as a simple price overwrite guarantees data integrity problems.

13. 867 ties back to 845 contracts. Vantage Supply's sample 867 includes PTD*SS***CT (contract number), REF*DH (DEA number), and AMT*CC (chargeback claim amount). The 867 references the same contracts the 845 creates. These are not independent — the contract system feeds the chargeback/rebate reporting.

14. Vantage Supply uses a prior VAN for AS2 connectivity. AS2 cert issued by a prior VAN (support@prior-van.example), Louvain-La-Neuve Belgium. Expires 2027-05-07. Acme Medical needs to provide their AS2 details for connectivity testing. Note: Orderful handles AS2 — Acme Medical doesn't need their own AS2 setup, Orderful's AS2 endpoint is what Vantage Supply will connect to.

15. Orderful handles guideline mapping — don't exchange separate specs. Vantage Supply (leader) defines guidelines, Orderful maps to Acme Medical's NetSuite. When Vantage Supply asked Acme Medical to "share your EDI specs for inbound documents," that's traditional point-to-point EDI thinking. On Orderful's platform, there's no direct connection. Clarify the Orderful model and ask about edge cases/statuses instead of spec exchange.

16. Custom SuiteScript delivery matches estimates. Abdallah delivered Core 4 scripts (850/855/856/810) in ~2 days after receiving Jordan's requirements. The 2-4 hours per TX type estimate was accurate. Custom fields are the long-pole — they require customer action in NS before scripts can run.
...
25. Minimal-first beats comprehensive-first for go-live speed. The May 14 meeting reduced the 855 from a 5-field design with automated hold triggers to a single free-text status field. The 856 went from Orderful EDI Carton records with carton hierarchy to just a tracking number. Total implementation surface dropped dramatically, and the customer was happier. a teammate's "2 weeks to first pass go-live" estimate is directly tied to this simplification. Over-engineering the first pass delays go-live and creates maintenance burden before anyone knows what matters.

## People

| Name | Role | Context |
|---|---|---|
| Jordan | Acme Medical customer contact | contact@acme.example |
| Kyle Coronel | Senior AE (Orderful) | Sold the deal, initial customer relationship |
| Scott Shields | Partner Ops Manager | Coordinates a contractor vendor, flagged cost/ARR concern |
| Selma | a contractor vendor Consulting PM | SI resource, installing connector |
| Abdallah | a contractor vendor Consulting Dev | Writing custom P2P SuiteScript |
| a teammate Foster | Orderful | Training, flagged SuiteApp limitation |
| Hiba Jabrane | Orderful | a contractor vendor oversight, initially misclassified |
| a teammate Riesman-Tremonte | Orderful Software Eng (DOT) | NS connector expert, P2P architecture guidance |
| Nikki Stephens | Orderful | Flagged medical distribution chargeback context |
| a teammate | Orderful VP Product | Took over May 11, driving to completion |
| Travita Dumas | Vantage Supply EDI team | Testing coordination, provided sample files and AS2 cert May 13 |
| a teammate | Orderful | CC'd on Vantage Supply testing email thread |
| Ana | Orderful | CC'd on Vantage Supply testing email thread |
