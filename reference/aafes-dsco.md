...
The outbound communication channel was initially pointing to the wrong AS2 destination, so outbound testing failed at delivery until the relationship was updated to the correct **DSCO/Rithum AS2** channel. **Always verify the outbound comm channel matches the DSCO/Rithum AS2 destination before testing.**
...
- **However:** the test 850 was successfully processed through NS despite the gaps — the SuiteApp's inbound flow worked for the test transaction
...
- Use the shared **"Rithum AS2"** connection in Orderful (reuses Orderful's shared cert already known to Rithum)
